...
if(isset($_POST["new_announcement"])){
    $aa_plugin_new_announcemenet = $_POST["new_announcement"];
    $this->add_annoucement( $aa_plugin_new_announcemenet );
    show_success_notice('Announcement saved.');
}

if(isset($_POST["announcements"])){
    $aa_plugin_ids_to_delete = $_POST["announcements"];
    $aa_plugin_ids_to_delete = implode( ', ', $aa_plugin_ids_to_delete );
    $this->delete_announcements( $aa_plugin_ids_to_delete );
...
function list_announcements_form( $aa_plugin_array ) {
    ...
    print '<tbody>';
    foreach( $aa_plugin_array as $aa_plugin_announcement ) {
        print '<tr><th class="check-column"><input type="checkbox" name="announcements[]" id="announcement' . $aa_plugin_announcement[0] . '" value="' . $aa_plugin_announcement[0] . '"></th>';
        print '<td><label for="announcement' . $aa_plugin_announcement[0] . '">' . $aa_plugin_announcement[1] . '</label></td></tr>';
    }
    print '</tbody></table>';
...
function show_success_notice( $aa_plugin_message ) {
    print "<div class='notice notice-success is- dismissible'><p>$aa_plugin_message</p></div>";
}
